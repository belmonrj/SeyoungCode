...
int CorCal_BBCS_FVTN()
{
  char SUB[16]="FVTN_BBCS";
  ...
  {
	if(i_cent==NCENTANA-1 && saveOption==1) sprintf(hname, "COR_BBCS_FVTN_C%d", 0);
	else sprintf(hname, "SIG_COR_BBCS_FVTN_C%d", i_cent);
	histo_original[i_cent] = new TH1D(hname, hname, H_BBC_phi, -0.5*PI, 1.5*PI);

	sprintf(hname, "SUB_BBCS_FVTN_C%d", i_cent);
  ...
  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	...
	{
	  TH2D *h1 = (TH2D*)rootopen->Get(Form("Real_dPhi_dEta_BBCS_FVTN_C%d_Z%d", i_cent, i_zvtx));
	  TH2D *h2 = (TH2D*)rootopen->Get(Form("Mix_dPhi_dEta_BBCS_FVTN_C%d_Z%d", i_cent, i_zvtx));
...
{
  TH1D *REF;
  if(subtraction==1) REF = (TH1D*)ref_open->Get(Form("COR_BBCS_FVTN_C%d", 0)); //Ref
