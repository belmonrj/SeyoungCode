...
int CorCal_FVT_N_S()
{
  ...
  href			= new TH1F("Reference correlation function", "href", H_FVTX_phi, -0.5*PI, 1.5*PI);
  TF1	*f2_tmp = new TF1("f2_tmp" , "1 + 2*[0]*cos(2*x)", -0.5*PI, 1.5*PI);
  TH1	*f_fit;

  f_fit= new TH1D("f_fit", "f_fit", H_FVTX_phi, -0.5*PI, 1.5*PI);
  ...
  {
	if(i_cent==NCENTANA-1 && saveOption==1) sprintf(hname, "COR_FVTN_FVTS_C%d", 0);
	else sprintf(hname, "SIG_FVTN_FVTS_C%d", i_cent);
	histo_original[i_cent] = new TH1D(hname, hname, H_FVTX_phi, -0.5*PI, 1.5*PI);

	sprintf(hname, "SUB_FVTN_FVTS_C%d", i_cent);
	histo_original_Sub[i_cent] = new TH1D(hname, hname, H_FVTX_phi, -0.5*PI, 1.5*PI);
  }
  ...
  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	...
	histo_original[i_cent]->SetAxisRange(0.95, 1.06, "Y");
	...
	histo_original_Sub[i_cent]->SetAxisRange(0.999, 1.0015, "Y");
  }
  ...
  TH2D *sumR  = new TH2D("sumR",  "Real pT sum histogram", H_FVTX_phi, -0.5*PI, 1.5*PI, H_FVTX_eta, -6, 0);
  TH2D *sumM  = new TH2D("sumM",  "Mix pT sum histogram", H_FVTX_phi, -0.5*PI, 1.5*PI, H_FVTX_eta, -6, 0);
  TH2D *tmpsumR  = new TH2D("tmpsumR",  " ", H_FVTX_phi, -0.5*PI, 1.5*PI, H_FVTX_eta, -6, 0);
  TH2D *tmpsumM  = new TH2D("tmpsumM",  " ", H_FVTX_phi, -0.5*PI, 1.5*PI, H_FVTX_eta, -6, 0);

  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	TH1D *sumR = (TH1D*)rootopen->Get(Form("Real_dPhi_FVT_S_N_C%d_Z0", i_cent));
	TH1D *sumM = (TH1D*)rootopen->Get(Form("Mix_dPhi_FVT_S_N_C%d_Z0", i_cent));

	for(int i_zvtx=1; i_zvtx<NZANA; i_zvtx++)
	{
	  TH1D *Tmpr = (TH1D*)rootopen->Get(Form("Real_dPhi_FVT_S_N_C%d_Z%d", i_cent, i_zvtx));
	  TH1D *Tmpm = (TH1D*)rootopen->Get(Form("Mix_dPhi_FVT_S_N_C%d_Z%d", i_cent, i_zvtx));
	  sumR->Add(Tmpr);
	  sumM->Add(Tmpm);
	...
	{
	  float val_R = sumR->GetBinContent(i_phi+1);
	  float val_M = sumM->GetBinContent(i_phi+1);
	  float ds_R  = sumR->GetBinError(i_phi+1);
	  float ds_M  = sumM->GetBinError(i_phi+1);
	  float sumRatio = 0;
	  float valRatio = 0;
	  float dsig = 0;
	...
	SavePar << i_cent << "\t" << c1[i_cent] << "\t" << c1err[i_cent] << "\t" << c2[i_cent] << "\t" << c2err[i_cent] << "\t" << chi2[i_cent] << "\t" << H_FVTX_phi-3 << "\t" << chi2[i_cent]/(H_FVTX_phi-3) << "\t" << c2[i_cent]/c1[i_cent] << "\n" ;
...
{
  TH1D *REF;
  if(subtraction==1) REF = (TH1D*)ref_open->Get(Form("COR_FVTN_FVTS_C%d", 0)); //Ref
  ...
  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	...
	SavePar_ref << i_cent << "\t" << c1ref[i_cent] << "\t" << c1ref_err[i_cent]  << "\t" << c2ref[i_cent] << "\t" << c2ref_err[i_cent] << "\t" << chi2[i_cent] << "\t" <<  H_FVTX_phi-3 << "\t" << chi2[i_cent]/(H_FVTX_phi-3) << "\t" << c2ref[i_cent]/c1ref[i_cent] << "\n";

	cout << c2[i_cent]-(PAR[0][i_cent]*c2[NCENTANA-1]) << "  " << c2ref[i_cent] << endl;

	SavePar_temp << i_cent << "\t" << c1ref[i_cent] << "\t" << c1ref_err[i_cent]  << "\t" << c2ref[i_cent]/PAR[1][i_cent] << "\t" << c2ref_err[i_cent] << "\t" << chi2[i_cent] << "\t" <<  H_FVTX_phi-3 << "\t" << chi2[i_cent]/(H_FVTX_phi-3) << "\t" << c2ref[i_cent]/c1ref[i_cent] << "\n";
