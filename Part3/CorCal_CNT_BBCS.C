...
int CorCal_CNT_BBCS()
{
  ...
  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	...
	{
	  if(i_cent==NCENTANA-1 && saveOption==1) sprintf(hname, "COR_CNT_BBCS_C%d_pT%d", 0, ipt_bin);
	  else sprintf(hname, "SIG_COR_CNT_BBCS_C%d_pT%d", i_cent, ipt_bin);
	  histo_original[i_cent][ipt_bin] = new TH1D(hname, hname, H_BBC_phi, -0.5*PI, 1.5*PI);

	  sprintf(hname, "SUB_CNT_BBCS_C%d_pT%d", i_cent, ipt_bin);
  ...
  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	...
	for(int ipt_bin=0; ipt_bin<NPTBIN; ipt_bin++)
	{
	  ...
	  {
		for(int i_eta=0; i_eta<H_BBCCNT_eta/2; i_eta++)
		{
