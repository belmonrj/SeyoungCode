...
int CorCal_BBCN_FVTS()
{
  char SUB[16]="BBCN_FVTS";
  ...
  {
	if(i_cent==NCENTANA-1 && saveOption==1) sprintf(hname, "COR_BBCN_FVTS_C%d", 0);
	else sprintf(hname, "SIG_COR_BBCN_FVTS_C%d", i_cent);
	histo_original[i_cent] = new TH1D(hname, hname, H_BBC_phi, -0.5*PI, 1.5*PI);

	sprintf(hname, "SUB_BBCN_FVTS_C%d", i_cent);
  ...
  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	...
	histo_original[i_cent]->SetAxisRange(0.98, 1.02, "Y");
  ...
  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	...
	{
	  TH2D *h1 = (TH2D*)rootopen->Get(Form("Real_dPhi_dEta_BBCN_FVTS_C%d_Z%d", i_cent, i_zvtx));
	  TH2D *h2 = (TH2D*)rootopen->Get(Form("Mix_dPhi_dEta_BBCN_FVTS_C%d_Z%d", i_cent, i_zvtx));
	...
	histo_original[i_cent]->Fit(fit_com[i_cent], "N");
...
{
  TH1D *REF;
  if(subtraction==1) REF = (TH1D*)ref_open->Get(Form("COR_BBCN_FVTS_C%d", 0)); //Ref
  ...
  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	...
	SavePar_ref << i_cent << "\t" << c1ref[i_cent] << "\t" << c1ref_err[i_cent]  << "\t" << c2ref[i_cent] << "\t" << c2ref_err[i_cent] << "\t" << chi2[i_cent] << "\t" <<  H_BBC_phi-3 << "\t" << chi2[i_cent]/(H_BBC_phi-3) << "\t" << c2ref[i_cent]/c1ref[i_cent] << "\n";

	cout << c2[i_cent]-(PAR[0][i_cent]*c2[NCENTANA-1]) << "  " << c2ref[i_cent] << endl;

	SavePar_temp << i_cent << "\t" << c1ref[i_cent] << "\t" << c1ref_err[i_cent]  << "\t" << c2ref[i_cent]/PAR[1][i_cent] << "\t" << c2ref_err[i_cent] << "\t" << chi2[i_cent] << "\t" <<  H_BBC_phi-3 << "\t" << chi2[i_cent]/(H_BBC_phi-3) << "\t" << c2ref[i_cent]/c1ref[i_cent] << "\n";
