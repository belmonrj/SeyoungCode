...
int CorCal_CNT_FVTS()
{
  ...
  for(int i_cent=0; i_cent<NCENTANA; i_cent++)
  {
	...
	{
	  if(i_cent==NCENTANA-1 && saveOption==1) sprintf(hname, "COR_CNT_FVTS_C%d_pT%d", 0, ipt_bin);
	  else sprintf(hname, "SIG_COR_CNT_FVTS_C%d_pT%d", i_cent, ipt_bin);
	  histo_original[i_cent][ipt_bin] = new TH1D(hname, hname, H_FVTX_phi, -0.5*PI, 1.5*PI);

	  sprintf(hname, "SUB_CNT_FVTS_C%d_pT%d", i_cent, ipt_bin);
	  ...
	  for(int i_zvtx=0; i_zvtx<NZANA; i_zvtx++)
	  {
		if(ipt_bin<Ch_bin)
		{
			...
			for (int i_phi=0; i_phi<H_FVTX_phi; i_phi++)
			{
		  		...
		  		for(int ieta=0; ieta<H_FVTX_eta/2; ieta++)
		  		{
